...
FarmEachShare, FarmPortableShare, HealthRemailShare, RootFarm, RemainFarm, dataTest ="", current_S_DELOY_UNIT,DetailError;
module.exports = 
{
	start: function(io) 
	{
		io.on('database', function(socket) 
		{
			...
			socket.on('S_CHECK_UNIT_IN_LOCATION', function (data)
			{
				...
				{             
					//Check tài khuyên
					database.query("SELECT unitType,Level,UnitOrder,Quality,HealthEach,FarmEach,FarmPortable,HealthRemain FROM `unitinlocations` where `idUnitInLocations` = '"+currentSENDCHECKUNITINLOCATION.idUnitInLocations
						...
						}else
						{
							...
							{							
								...
								database.query("SELECT Quality,idUNBase FROM `unitinbase` where `UserName` = '"+currentSENDCHECKUNITINLOCATION.UserName
										...
										{										
											...
											if(parseFloat(HealthRemailShare) === parseFloat(HealthEachShare)*parseFloat(QualityShare))
											{
												...
												if ((parseFloat(rows[0].Quality)+parseFloat(currentSENDCHECKUNITINLOCATION.Quality)) === parseFloat(currentSENDCHECKUNITINLOCATION.QualityAfterMerge)) 
												{
													...
													database.query("UPDATE unitinbase SET Quality = '"+parseFloat(currentSENDCHECKUNITINLOCATION.QualityAfterMerge)
															...
															{																				
																...
																{            
																	if (parseFloat(data)===1) 
																	{
																			...
																			database.query("SELECT idBase,Farm FROM `userbase` where `UserName` = '"+currentSENDCHECKUNITINLOCATION.UserName
																				+"'AND `numberBase` = '"+currentSENDCHECKUNITINLOCATION.numberBase
												...
												{
													//gửi mail
													database.query("SELECT * FROM `unitinlocations` where `UserName` = '"+currentSENDCHECKUNITINLOCATION.UserName+"'",function(error, rows,field)
													{
																...
																database.query("SELECT Quality,idUNBase FROM `unitinbase` where `UserName` = '"+currentSENDCHECKUNITINLOCATION.UserName
																	...
																	}else
																	{
																		...
																		{													
																			...
																			{
																				//gửi mail
																				database.query("SELECT * FROM `unitinlocations` where `UserName` = '"+currentSENDCHECKUNITINLOCATION.UserName
																					+"'",function(error, rows,field)
																					{
