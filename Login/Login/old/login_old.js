...
var d,currentTime, redisDatas = [],currentUser=[], redisarray = [], lastPosition="",arrayTimeResetserver=0,hourReset=0,minuteReset = 0,secondReset=0,
...
module.exports = {
	start: function(io) 
	{
		...
		{   				
			...
			{						
				var s,arrayOnlineStatus =[],d = new Date(), currentTime = Math.floor(d.getTime() / 1000);		
				//lấy dữ liệu login
											...
											if (rows.length>0) 
											{
												for (var i = 0; i < rows.length; i++)
												{
													...
													connection.query("SELECT * FROM `unitinbase` WHERE `UserName`='"+rows[i].UserName
														...
														{	
															if (rows.length > 0 
																&& (parseFloat(currentTime) < parseFloat(rows[0].TimeCompleteUnitMoveSpeed))
																) 
															{																					
																
																connection.query("UPDATE unitinbase SET TimeWaitUnitMoveSpeed ='"+ (parseFloat(rows[0].TimeCompleteUnitMoveSpeed)-parseFloat(currentTime))			                					        						                				                						                				                			
	...
	if (rows.length>0) 
	{
		...
		{	
			...
			if ( parseFloat(rows[i].FarmTransferCompleteTime)> parseFloat(currentTime))  
			{
				...
				connection.query("UPDATE `unitinlocations` SET TransferCompleteTotalTime = '"+(parseFloat(rows[i].FarmTransferCompleteTime)-parseFloat(currentTime)) 
				...
				connection.query("UPDATE userbase,unitinlocations SET unitinlocations.FarmPortable = unitinlocations.FarmPortable + unitinlocations.FarmWait,userbase.TransferUnitOrderStatus = '"+ 0								
					...
					+"' AND unitinlocations.UnitOrder ='"+rows[i].UnitOrder+"'",function(error, result, field)
					{
						...
						functions.writeLogErrror(DetailError);	
											    ...
											    connection.query("SELECT * FROM `userbase` WHERE `UserName` = '"+currentUser.name
											    	...
											    	}else
											    	{
											    		for (var i = 0; i < rows.length; i++)
											    		{
															...
															if ( parseFloat(rows[index].TimeWaitSum)> parseFloat(currentTime))  
															{
																connection.query("UPDATE userbase SET TimeWait = '"+parseFloat(parseFloat(rows[index].TimeWaitSum)-parseFloat(currentTime)) 
																	...
																	else
																	{
																		GetUpGradeItem(rows[index].UserName, 
																			...
																			parseFloat(parseFloat(rows[index].TimeWaitSum)-parseFloat(currentTime)), io);
																	}
																});
															}else
															{
										        ...
										        }else
										        {
										        	for (var i = 0; i < rows.length; i++)
										        	{
										        		var index = i; 
											    			...
											    			if ( parseFloat(rows[i].TimeCompleteResourceTransferFromFriend)> parseFloat(currentTime))  
											    			{
											    				connection.query("UPDATE userbase SET TimeRemainResourceTransferFromFriend = '"+(parseFloat(rows[i].TimeCompleteResourceTransferFromFriend)-parseFloat(currentTime)) +"' where UserName = '"+rows[i].UserName
											    			...
											    			//#region Thu hoạch	
												    			...
												    			connection.query("SELECT A.Farm,A.LvFarm,A.UserName,A.numberBase,A.FarmHarvestTime,A.CurrentLvFarm, B.HarvestPerHour, B.HarvestContainer FROM userbase AS A INNER JOIN resourceupfarm AS B ON B.Level =  A.CurrentLvFarm WHERE A.UserName = '"+rows[index].UserName
												    				...
												    				{				
												    					...
												    					{					    						
												    						if ((parseFloat(currentTime) - parseFloat(rows[0].FarmHarvestTime)) >= parseFloat(rows[0].HarvestContainer)) 
												    						{																													
														                	...
														                	{				                			
														                		MineHarvestServer =(parseFloat(currentTime) - parseFloat(rows[0].FarmHarvestTime))*(parseFloat(rows[0].HarvestPerHour)/3600);
																...
																connection.query("SELECT A.Wood,A.LvWood,A.UserName,A.numberBase,A.WoodHarvestTime,A.CurrentLvWood, B.HarvestPerHour, B.HarvestContainer FROM userbase AS A INNER JOIN resourceupwood AS B ON B.Level =  A.CurrentLvWood WHERE A.UserName = '"+rows[index].UserName
																	...
																	{																	
																		...
																		{				    						
																			if ((parseFloat(currentTime) - parseFloat(rows[0].WoodHarvestTime)) >= parseFloat(rows[0].HarvestContainer)) 
																			{																													
														                		//update tài nguyên còn lại của base					                		
														                	...
														                	{				                			
														                		MineHarvestServer =(parseFloat(currentTime) - parseFloat(rows[0].WoodHarvestTime))*(parseFloat(rows[0].HarvestPerHour)/3600);
														                		if ((MineHarvestServer % 1 ) >= 0.5)
														                		{
														                			MineHarvestServer=Number((MineHarvestServer).toFixed(0));
														                		}else
																...
																connection.query("SELECT A.Stone,A.LvStone,A.UserName,A.numberBase,A.StoneHarvestTime,A.CurrentLvStone, B.HarvestPerHour, B.HarvestContainer FROM userbase AS A INNER JOIN resourceupstone AS B ON B.Level =  A.CurrentLvStone WHERE A.UserName = '"+rows[index].UserName
																	...
																	{																	
																		...
																		{				    						
																			if ((parseFloat(currentTime) - parseFloat(rows[0].StoneHarvestTime)) >= parseFloat(rows[0].HarvestContainer)) 
																			{																													
														                		//update tài nguyên còn lại của base					                		
														                	...
														                	{				                			
														                		MineHarvestServer =(parseFloat(currentTime) - parseFloat(rows[0].StoneHarvestTime))*(parseFloat(rows[0].HarvestPerHour)/3600);
														                		if ((MineHarvestServer % 1 ) >= 0.5)
														                		{
														                			MineHarvestServer=Number((MineHarvestServer).toFixed(0));
														                		}else
																...
																connection.query("SELECT A.Metal,A.LvMetal,A.UserName,A.numberBase,A.MetalHarvestTime,A.CurrentLvMetal, B.HarvestPerHour, B.HarvestContainer FROM userbase AS A INNER JOIN resourceupmetal AS B ON B.Level =  A.CurrentLvMetal WHERE A.UserName = '"+rows[index].UserName
																	...
																	{				
																		...
																		{	
																			if ((parseFloat(currentTime) - parseFloat(rows[0].MetalHarvestTime)) >= parseFloat(rows[0].HarvestContainer)) 
																			{																													
														                		//update tài nguyên còn lại của base					                		
														                	...
														                	{				                			
														                		MineHarvestServer =(parseFloat(currentTime) - parseFloat(rows[0].MetalHarvestTime))*(parseFloat(rows[0].HarvestPerHour)/3600);
														                		if ((MineHarvestServer % 1 ) >= 0.5)
														                		{
														                			MineHarvestServer=Number((MineHarvestServer).toFixed(0));
														                		}else
															...
															{						
																if (rows.length > 0 && (parseFloat(currentTime)<parseFloat(rows[0].TimeCompleteResourceMoveSpeed))) 
																{	
																	connection.query("UPDATE userbase SET TimeWaitResourceMoveSpeed = '"+ (parseFloat(rows[0].TimeCompleteResourceMoveSpeed)-parseFloat(currentTime))								                							                						                		
																		+"'where UserName = '"+rows[0].UserName
																		+"'AND numberBase = '"+parseFloat(rows[0].numberBase)+"'",function(error, result, field)
																		{
																	...
																	&& (parseFloat(currentTime)>=parseFloat(rows[0].TimeCompleteResourceMoveSpeed))
												...
												var query = database.query("SELECT DetailMore FROM `task` WHERE DetailTask ='ResetMine'",function(error, rows,field)
												{
												...
												}else
												{
													...
													{	
														...
														{		        						
															connection.query("UPDATE unitwaitinbase SET timeRemain = '"+(parseFloat(rows[i].timeComplete)-parseFloat(currentTime))
																+"' WHERE UserName = '"+rows[i].UserName
																+"' AND numberBase = '"+parseFloat(rows[i].numberBase)
												...
												{											
													for (var i = 0; i < rows.length; i++)
													{
														...
														B2 = parseFloat(B[1]);
															...
															{						    							
																if( (parseFloat(currentTime) - parseFloat(rows[index].timeClick)) < parseFloat(rows[index].TimeMoveComplete) )
																{
																	timeMoves=0;
																	X= A1+((parseFloat(currentTime) - parseFloat(rows[index].timeClick))*(B1-A1))/parseFloat(rows[index].TimeMoveComplete);
																	Z= A2+((parseFloat(currentTime) - parseFloat(rows[index].timeClick))*(B2-A2))/parseFloat(rows[index].TimeMoveComplete);
																	timeMoves = parseFloat(rows[index].TimeMoveTotalComplete) - parseFloat(currentTime);				    							
									    							...
									    							{							    								
									    								...
									    								if (parseFloat(timeMoves) >= (1/parseFloat(rows[index].MoveSpeedEach))) 
									    								{
									    									...
									    									connection.query("UPDATE unitinlocations SET Position = '"+ X+","+Z 	  
									    										...
									    										+"',TimeSendToredisData = '"+ (1/parseFloat(rows[index].MoveSpeedEach)+parseFloat(currentTime))
									    							...
									    							}else
									    							{
									    									...
									    									&& ((parseFloat(rows[index].FarmPortable) - FarmConsumeInOffline) >= parseFloat(rows[index].Quality))) 
									    								{
									    									...
									    									var query = database.query("UPDATE unitinlocations SET Position ='"+X+","+Z
									    										...
									    										+"',TimeMoveComplete = '"+parseFloat(timeMoves)
									    										+"',TimeSendToredisData = '"+ ((1/parseFloat(rows[index].MoveSpeedEach))+parseFloat(currentTime))
									    										+"',CheckMoveOff=0,TimeMoveTotalComplete = '"+(parseFloat(timeMoves)+parseFloat(currentTime))
									    										+"',FarmPortable = '"+parseFloat(parseFloat(rows[index].FarmPortable) - parseFloat(FarmConsumeInOffline))
									    										+"',timeClick = '"+ parseFloat(currentTime)
																					...
																					{													
																						...
																						{																												
																								...
																								connection.query("INSERT INTO `userbase`(`idBase`, `UserName`, `MaxStorage`, `Position`, `LvCity`, `Farm`, `Wood`, `Stone`, `Metal`, `CreateTime`, `numberBase`,`sizeUnitInBase`, `checkResetMine`, `UpgradeWait`, `ResourceMoveSpeed`, `UnitMoveSpeed`, `UnitNumberLimitTransfer`) VALUES ('"
																									+""+"','"+currentUser.name+"','"+10000+"','"+newLocation+"','"+1+"','"+rows[0].FarmReady+"','"+rows[0].WoodReady+"','"+rows[0].StoneReady+"','"+rows[0].MetalReady+"','"+currentTime+"','"+rows[0].numberBase+"','"+0+"','"+0+"','"+rows[0].UpgradeWait+"','"+rows[0].ResourceMoveSpeed+"','"+rows[0].UnitMoveSpeed+"','"+rows[0].UnitNumberLimitTransfer+"')",function(error, result, field)
																									{
											...
											connection.query("UPDATE users SET timeLogin = ?,idSocket=?,timeLogout =? WHERE UserName = ?", [currentTime,socket.id,0, currentUser.name],function(error, result, field)
											{													
...
{
	if ( parseFloat(currentTime)>=parseFloat(dataGuild[0].TimeReset) 
...
{	
	...
	var query = database.query("UPDATE userbase SET "+FarmHarvestTime+" = '"+parseFloat(currentTime)
...
{   	
	var query = database.query("SELECT `idUnitInLocations`, `UserName`, `UnitOrder`, `UnitType`, `Quality`, `FarmPortable`, `Health`, `HealthEach`, `HealthRemain`, `Damage`, `DamageEach`, `Defend`, `DefendEach`, `FarmEach`, `FarmWait`, `FarmTransferCompleteTime`, `TransferCompleteTotalTime`, `numberBaseTransfer`,  `PositionLogin` as `Position`, `Level`, `PositionClick`, `MoveSpeedEach`, `TimeCheck`, `timeClick`, `CheckLog`, `CheckOnline`, `CheckCreate`, `TimeFight`, `CheckFight`, `userFight`, `FightRadius`, `FightRadiusPosition`, `FightRadiusPosition1`, `FightRadiusPosition2`, `DistanceMoveLogoff`, `CheckMove`, `TimeMoveComplete`, `TimeMoveTotalComplete`, `CheckMoveOff` FROM `unitinlocations` WHERE `UserName`='"+UserName+"' AND `TransferCompleteTotalTime`>0",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `users` WHERE 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `chatting` WHERE `GuildName`='"+GuildName+"' AND `ServerTime` >= '"+parseFloat(TimeCancelGuild)+"' ",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `chatting` WHERE `ToUserName`='"+UserName+"' AND `CheckCloseMessPrivate` = 1 ",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `blacklist` WHERE `UserName`='"+UserName+"'",function(error, rows,field)
	{
		if (error) {
...
{   
	...
	var query = database.query("SELECT * FROM `blacklist` WHERE `WithUserName`='"+UserName+"'",function(error, rows,field)
	{
		if (error) {
...
{   
	var query = database.query("SELECT * FROM `resourcetodiamond` WHERE 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `policys` where GuildName='"+GuildName+"'",function(error, rows,field)
	{
		if (error) {
...
{   		
	...
	var query = database.query("SELECT DetailTimeBlock,CheckBLockForever FROM `users` WHERE `UserName`='"+currentUser.name+"'",function(error, rows,field)
	{
		if (!!error) {DetailError =("login :Error in the query 95");
	...
	else
	{
		if (rows.length>0) 
		{
			d = new Date(), currentTime = Math.floor(d.getTime() / 1000),TimeCheckSend = 1, TimeResetToredisData = 0;		
			console.log("User đăng nhập: "+ currentUser.name);
			console.log("Thời gian hiện tại: "+ parseFloat(currentTime));
			...
			if (parseFloat(rows[0].DetailTimeBlock) > parseFloat(currentTime)) 
			{
				...
				TimeCheckSend = 0;
				TimeSendResetUser = TimeResetToredisData - parseFloat(currentTime);
					//Khóa tạm thời
				...
				}else
				{
					...
					var query = database.query("SELECT * FROM `users` WHERE `UserName`='"+UserName+"'",function(error, rows,field)
					{	
						if (error) {
...
{   	
	var query = database.query("SELECT * FROM `guildlistmember` WHERE ActiveStatus = 0 AND GuildName = '"+GuildName+"'",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `guildlistmember` WHERE `InviteByUser`!='' AND MemberName = '"+UserName+"'",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `guildlistmember` WHERE (ActiveStatus=2 OR ActiveStatus=1) AND GuildName = '"+GuildName+"'",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `guildlist` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupguild` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourcebuybase` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT `UserNameFriendB` FROM `addfriend` WHERE `UserNameFriendA`='"+UserName+"' AND `ActiveStatus`=0",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT `UserNameFriendA` FROM `addfriend` WHERE `UserNameFriendB`='"+UserName+"' AND `ActiveStatus`=0",function(error, rows,field)
	{		
		if (error) {
...
{   	
	var query = database.query("SELECT  * FROM `unitinbase` WHERE `UserName` ='"+UserName+"'",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT `idUnitInLocations`, `UserName`, `UnitOrder`, `UnitType`, `Quality`, `FarmPortable`, `Health`, `HealthEach`, `HealthRemain`, `Damage`, `DamageEach`, `Defend`, `DefendEach`, `FarmEach`, `FarmWait`, `FarmTransferCompleteTime`, `TransferCompleteTotalTime`, `numberBaseTransfer`,  `PositionLogin` as `Position`, `Level`, `PositionClick`, `MoveSpeedEach`, `TimeCheck`, `timeClick`, `CheckLog`, `CheckOnline`, `CheckCreate`, `TimeFight`, `CheckFight`, `userFight`, `FightRadius`, `FightRadiusPosition`, `FightRadiusPosition1`, `FightRadiusPosition2`, `DistanceMoveLogoff`, `CheckMove`, `TimeMoveComplete`, `TimeMoveTotalComplete`, `CheckMoveOff` FROM `unitinlocations` WHERE 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourcebuyunit` WHERE 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupcity` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupswordman` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupfarm` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupwood` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupstone` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupmetal` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupBowman` where 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT `idresourceupGranary`, `Level`, `Farm`, `Wood`, `Stone`, `Metal`, `Diamond`, `TimeUp`,`MaxStorage`   FROM `resourceupgranary` WHERE 1",function(error, rows,field)
	{
		if (error) {
...
{   	
	var query = database.query("SELECT * FROM `resourceupMarket` where 1",function(error, rows,field)
	{
		if (error) {
