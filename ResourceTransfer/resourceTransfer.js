...
// 	});
// }
exports.updateTimeTransferRes = function updateTimeTransferRes (rows,currentTime) {
	database.query("SELECT UserName,numberBase,FarmWait,WoodWait,StoneWait,MetalWait,TimeCompleteResourceMoveSpeed,ResourceTransferToBase FROM userbase WHERE UserName = '"
		+rows.UserName+"'AND numberBase = '"+rows.numberBase+"'",function(error,result){
			if (!!error){DetailError =('resourceTransfer :Error updateTimeTransferRes '+rows.UserName);functions.writeLogErrror(DetailError);}
			if (result.length>0) {
				for (var i = 0; i < result.length; i++) {
					if ((parseFloat(currentTime)<parseFloat(result[i].TimeCompleteResourceMoveSpeed))){
						updateTimeTranfer (result[i],parseFloat(result[i].TimeCompleteResourceMoveSpeed)-parseFloat(currentTime));						
					}
					else if ((parseFloat(currentTime)>=parseFloat(result[i].TimeCompleteResourceMoveSpeed))
						&& (parseFloat(result[i].TimeCompleteResourceMoveSpeed)>0)) {
						selectData(result);
...
//cập nhật thời gian vận chuyển tài nguyên
exports.updateTimeTransferResToFriend = function updateTimeTransferResToFriend (rowsData,currentTime) {
	if (parseFloat(rowsData.TimeCompleteResourceTransferFromFriend)>parseFloat(currentTime)) {
		database.query("UPDATE userbase SET TimeRemainResourceTransferFromFriend = '"+(parseFloat(rowsData.TimeCompleteResourceTransferFromFriend)-parseFloat(currentTime)) 
			+"' WHERE UserName = '"+rowsData.UserName+"'AND numberBase = '"+rowsData.numberBase+"'",function(error){
				if(!!error){DetailError =('resourceTransfer :Error updateTimeTransferResToFriend '+rowsData.UserName);functions.writeLogErrror(DetailError);	
			}
		});
	}	
}
//Cập nhật thời gian gửi farm từ base đến unit location
exports.updateFarmFromBaseToUnit = function updateFarmFromBaseToUnit (currentUser,currentTime) {
	database.query("SELECT * FROM `unitinlocations` WHERE `UserName` = '"+currentUser.name+"'",function(error, rows){
		if (!!error){DetailError = ('resourceTransfer: Error select from unitinlocations '+currentUser.name);functions.writeLogErrror(DetailError);}
		if (rows.length>0) {
			for (var i = 0; i < rows.length; i++) {
				if (parseFloat(rows[i].FarmTransferCompleteTime)> parseFloat(currentTime)) {
					database.query( updateTimeTransfer (rows[i],currentTime) ,function(error, result)
					{
						if(!!error){DetailError =('resourceTransfer:Error updateTimeTransfer'); functions.writeLogErrror(DetailError);}
					});
				}
				else if ((parseFloat(rows[i].FarmTransferCompleteTime)<parseFloat(currentTime))&&(parseFloat(rows[i].FarmTransferCompleteTime)> 0)) {
					database.query(updateTimeTransferFinished (rows[i]),function (error) {
						if(!!error){DetailError =('resourceTransfer: Error updateTimeTransferFinished '+ currentUser.name); functions.writeLogErrror(DetailError);}
					});
				}
			}
		}
...
//trường hợp finish time
function updateTimeTransferFinished (rows) {
	var farmUpdate =rows.FarmPortable + rows.FarmWait;
	console.log("farmUpdate:_"+farmUpdate);
	return "UPDATE userbase,unitinlocations SET unitinlocations.FarmPortable = '"+farmUpdate+"',userbase.TransferUnitOrderStatus = '0'"								
	+",unitinlocations.FarmWait = '0'"                			
	+",unitinlocations.FarmTransferCompleteTime = '0'"
	+",unitinlocations.TransferCompleteTotalTime = '0'"				                				                						                				                			
	+" WHERE userbase.UserName = unitinlocations.UserName AND userbase.UserName ='"+rows.name
	+"' AND userbase.numberBase = '"+rows.numberBaseTransfer
	+"' AND unitinlocations.UnitOrder ='"+rows.UnitOrder+"'";
}
//trường hợp còn thời gian
function updateTimeTransfer (rows,currentTime) {
	return "UPDATE `unitinlocations` SET TransferCompleteTotalTime = '"+(parseFloat(rows.FarmTransferCompleteTime)-parseFloat(currentTime)) 
	+"' WHERE UserName = '"+rows.UserName+"' AND UnitOrder = '"+rows.UnitOrder+"'";
}
function selectData (rows) {
	connection.query("SELECT A.UserName,A.numberBase,A.Farm,A.Wood,A.Stone,A.Metal,A.FarmWait, A.WoodWait, A.StoneWait, A.MetalWait,A.TimeCompleteResourceMoveSpeed,A.ResourceTransferToBase,C.MaxStorage FROM userbase AS A INNER JOIN resourceupgranary AS C ON C.Level = A.LvGranary WHERE A.UserName = '"+rows[0].UserName+"'AND A.numberBase = '"+parseFloat(rows[0].numberBase)
		+"' UNION ALL SELECT A.UserName,A.numberBase,A.Farm,A.Wood,A.Stone,A.Metal,A.FarmWait, A.WoodWait, A.StoneWait, A.MetalWait,A.TimeCompleteResourceMoveSpeed,A.ResourceTransferToBase,C.MaxStorage FROM userbase AS A INNER JOIN resourceupgranary AS C ON C.Level = A.LvGranary WHERE A.UserName = '"+rows[0].UserName+"'AND A.numberBase = '"+parseFloat(rows[0].ResourceTransferToBase)+"'",function(error,result){
			if (!!error){DetailError =('resourceTransfer :Error selectData '+rows.UserName);	functions.writeLogErrror(DetailError);}
			if (result.length>0) {
				farmRes (result);
				woodRes (result);
				stoneRes (result);
...
//metal
function metalRes (result) {
	var TransferSurpass = (parseFloat(result[1].Metal) + parseFloat(result[0].MetalWait)) - parseFloat(result[1].MaxStorage);
	var TransferRemain = parseFloat(result[0].MetalWait) - TransferSurpass;
	if ((parseFloat(result[1].Metal) + parseFloat(result[0].MetalWait)) >=  parseFloat(result[1].MaxStorage)) {		
		updateMetalTransfer (result,TransferRemain);	
	}else{
		updateMetalTransfer (result,TransferSurpass);
		updateSenderBase(result,MetalWait);
	}
}
function updateMetalTransfer (result,QualityTransfer) {
	database.query("UPDATE userbase SET Metal = Metal +'"+parseFloat(QualityTransfer)									
		+"' WHERE UserName = '"+result[0].UserName+"'AND numberBase = '"+parseFloat(result[1].numberBase)+"'",function(error){
			if (!!error){DetailError =('resourceTransfer :Error updateMetalTransfer '+result.UserName);functions.writeLogErrror(DetailError);}
		});
}
//stone
function stoneRes (result) {
	var TransferSurpass = (parseFloat(result[1].Stone) + parseFloat(result[0].StoneWait)) - parseFloat(result[1].MaxStorage);
	var TransferRemain = parseFloat(result[0].StoneWait) - TransferSurpass;
	if ((parseFloat(result[1].Stone) + parseFloat(result[0].StoneWait)) >=  parseFloat(result[1].MaxStorage)) {		
		updateStoneTransfer (result,TransferRemain);	
	}else{
		updateStoneTransfer (result,TransferSurpass);
		updateSenderBase(result,StoneWait);
	}
}
function updateStoneTransfer (result,QualityTransfer) {
	database.query("UPDATE userbase SET Stone = Stone +'"+parseFloat(QualityTransfer)									
		+"' WHERE UserName = '"+result[0].UserName+"'AND numberBase = '"+parseFloat(result[1].numberBase)+"'",function(error){
			if (!!error){DetailError =('resourceTransfer :Error updateStoneTransfer '+result.UserName);functions.writeLogErrror(DetailError);}
		});
}
//wood
function woodRes (result) {
	var TransferSurpass = (parseFloat(result[1].Wood) + parseFloat(result[0].WoodWait)) - parseFloat(result[1].MaxStorage);
	var TransferRemain = parseFloat(result[0].WoodWait) - TransferSurpass;
	if ((parseFloat(result[1].Wood) + parseFloat(result[0].WoodWait)) >=  parseFloat(result[1].MaxStorage)) {		
		updateWoodTransfer(result,TransferRemain);	
	}else{
		updateWoodTransfer(result,TransferSurpass);
		updateSenderBase(result,WoodWait);
	}
}
function updateWoodTransfer (result,QualityTransfer) {
	database.query("UPDATE userbase SET Wood = Wood +'"+parseFloat(QualityTransfer)									
		+"' WHERE UserName = '"+result[0].UserName+"'AND numberBase = '"+parseFloat(result[1].numberBase)+"'",function(error){
			if (!!error){DetailError =('resourceTransfer :Error updateWoodTransfer '+result.UserName);functions.writeLogErrror(DetailError);}
		});
}
//farm
function farmRes (result) {
	var FarmTransferSurpass = (parseFloat(result[1].Farm) + parseFloat(result[0].FarmWait)) - parseFloat(result[1].MaxStorage);
	var FarmTransferRemain = parseFloat(result[0].FarmWait) - FarmTransferSurpass;
	if ((parseFloat(result[1].Farm) + parseFloat(result[0].FarmWait)) >=  parseFloat(result[1].MaxStorage)) {		
		updateFarmTransfer (result,FarmTransferRemain);	
	}else{
		updateFarmTransfer (result,FarmTransferSurpass);
		updateSenderBase(result,FarmWait);
	}
}

function updateFarmTransfer (result,QualityTransfer) {
	database.query("UPDATE userbase SET Farm = Farm +'"+parseFloat(QualityTransfer)									
		+"' WHERE UserName = '"+result[0].UserName+"'AND numberBase = '"+parseFloat(result[1].numberBase)+"'",function(error){
			if (!!error){DetailError =('resourceTransfer :Error updateFarmTransfer '+result.UserName);functions.writeLogErrror(DetailError);}
		});
}

function updateTimeTranfer (resultData,TimeCommpleteMove) {
	database.query("UPDATE userbase SET TimeWaitResourceMoveSpeed = '"+ parseFloat(TimeCommpleteMove)								                							                						                		
		+"'WHERE UserName = '"+resultData.UserName+"'AND numberBase = '"+parseFloat(resultData.numberBase)+"'",function(error){
			if (!!error){DetailError =('resourceTransfer :Error updateTimeTranfer '+resultData.UserName);functions.writeLogErrror(DetailError);}
		});
}

function updateSenderBase (rows,setRes) {
	connection.query("UPDATE userbase SET "+setRes+" = 0, TimeCompleteResourceMoveSpeed = 0,ResourceTransferToBase = 0,TimeWaitResourceMoveSpeed=0 where UserName = '"+rows[0].UserName				    		
		+"'AND numberBase = '"+parseFloat(rows[0].numberBase)+"'",function(error){
			if (!!error){DetailError =('resourceTransfer :Error updateSenderBase '+rows.UserName);	functions.writeLogErrror(DetailError);}
		});
}
...
//     		var query = database.query("SELECT * FROM `unitinlocations` WHERE FarmTransferCompleteTime > 0 AND FarmTransferCompleteTime <= '"+parseFloat(createPositionTimelast)+"'",function(error, rows,field)
// 			{
...
// 				  			var index = i;
// 				  			//hoàn tất thời gian
// 		    				var query = database.query("UPDATE userbase,unitinlocations SET unitinlocations.FarmPortable = unitinlocations.FarmPortable + unitinlocations.FarmWait,userbase.TransferUnitOrderStatus = '"+ 0								
// 	                			+"',unitinlocations.FarmWait = '"+ 0                			
// 	                			+"',unitinlocations.FarmTransferCompleteTime = '"+0
// 	                			+"',unitinlocations.TransferCompleteTotalTime = '"+0				                				                						                				                			
...
// 						                	var query = database.query("SELECT `idUnitInLocations`,`UserName`,`HealthEach`,`HealthRemain`,`FarmPortable`, `Damage`,`DamageEach`, `Defend`,`DefendEach`, `Quality`,`UnitOrder`,`CheckOnline`,`userFight`,`CheckFight` FROM `unitinlocations` WHERE 1",function(error, rows,field)
// 											{
...
//     		var query = database.query("SELECT * FROM `userbase` WHERE TimeCompleteResourceTransferFromFriend > 0 AND  TimeCompleteResourceTransferFromFriend <= '"+parseFloat(createPositionTimelast)+"'",function(error, rows,field)
// 			{
...
// 				  			var query = database.query("SELECT * FROM `userbase` WHERE UserName = '"+arrayUserTransfer[index].ResourceTransferToUserNameOfFriend
...
// 											var query = database.query("UPDATE userbase SET Farm = '"+ (parseFloat(FarmRemain))
...
// 										else
// 										{
// 											var query = database.query("UPDATE userbase SET Farm = Farm + '"+ (parseFloat(arrayUserTransfer[index].FarmWaitFromFriend))
