...
var currentSENDFARMCONSUME,d,createPositionTimelast,arrayDetailBase =[], clients	= [];
...
var d,createPositionTimelast, arrayFightOffline = [], arrayFightOnlinevsOffline = [], arrayFightOnlinevsOnline = [], arrayAvsB = [],arrayRequestJoinGuild=[];
															...
															connection.query("SELECT * FROM `guildlistmember` WHERE GuildName = '"+rows[0].GuildName+"'",function(error, rows,field)
															{
																...
																}else
																{
															     ...
															     //            	});
																				...
																				socket.broadcast.to(clients[clients.findIndex(item => item.name === arrayNotisyStatus[i].MemberName)].idSocket).emit('RECEIVESTATUS',
																				{
																					Status : 1,
																					TimeDetail : dt.format('d-m-Y H:M:S')+" "+ new Date().toString().slice(25, 33),	
																					UserName : currentUser.name,																					
																													...
																													for (var i = 0; i < arrayOnlineStatus.length; i++) 
																											  		{
																											  			...
																											  			{	
																										                	...
																										                	socket.emit('RECEIVESTATUSFORALL',
																															{
																																Status : 1,	
																																UserName : currentUser.name,
																																idUnitInLocations: arrayUnitLocationsComplete,																					
																															...
																															socket.broadcast.to(clients[clients.findIndex(item => item.name === arrayOnlineStatus[i].UserName)].idSocket).emit('RECEIVESTATUSFORALL',
																															{
																																Status : 1,	
																																UserName : currentUser.name,
																																idUnitInLocations: arrayUnitLocationsComplete,																					
