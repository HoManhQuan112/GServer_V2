	    		...
	    		{	    			
	    			...
	    			if (arrayAvsB.length > 0) 
					{
						...
						{													
							...
							{								
								gameServer.redisarray.splice(gameServer.redisarray.findIndex(item => item.idUnitInLocations === parseFloat(arrayAvsBFightOut[index].idUnitInLocationsA)), 1);							
							}	
									...
									{	
											...
											+"',checkFight = 0, userFight = '' WHERE idUnitInLocations = ?",[parseFloat(A)],function(error, result, field)
										{
											...
											{	
												...
												gameServer.redisarray[gameServer.redisarray.findIndex(item => item.idUnitInLocations === parseFloat(A))].FarmPortable = parseFloat(gameServer.redisarray[gameServer.redisarray.findIndex(item => item.idUnitInLocations === parseFloat(A))].FarmPortable)+parseFloat(gameServer.redisarray[gameServer.redisarray.findIndex(item => item.idUnitInLocations === parseFloat(B))].FarmPortable);					
												...
												//xóa torng redis	
												gameServer.redisarray.splice(gameServer.redisarray.findIndex(item => item.idUnitInLocations === parseFloat(A)), 1);					
