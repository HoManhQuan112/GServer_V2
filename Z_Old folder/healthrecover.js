...
var currentSENDHEALTHRECOVER,d,createPositionTimelast;
    ...
    start: function(io) 
    {
        io.on('connection', function(socket) 
        {
			...
			{			
				...
				console.log("============SENDHEALTHRECOVER======"+"_"
				...
				{		
					connection.query("SELECT HealthRemain,FarmEach,Quality,HealthEach, FarmPortable FROM `unitinlocations` WHERE `idUnitInLocations`='"+currentSENDHEALTHRECOVER.idUnitInLocations+"'",function(error, rows,field)
		        	{
						...
						if(!!error){console.log('Error in the query 376');
						}else
						{
							if(rows.length > 0)
							{							
								...
								{								
									...
									{
										if(!!error){console.log('Error in the query 377');
										}else
										{
											...
											{	
												...
												{
													if (!!error){console.log('Error in the query 378');
													}else
													{	
								...
								{
									console.log('Mail Error in the database 6');
									//gửi mail báo cáo								
									...
									socket.emit('RECEIVEHEALTHRECOVER',
									{
										...
										Farm:parseInt(rows[0].FarmPortable,10),
